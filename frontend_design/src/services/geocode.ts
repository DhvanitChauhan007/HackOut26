...
// Landmark keywords for instant client-side resolution
...
export async function geocodeAddress(
  ...
  if (!address || typeof address !== "string") {
    return { lat: 23.188408, long: 72.627969 };
  }
  ...
  // 1. Instant check for known landmark / locality patterns
  ...
  // 2. Check geocode_cache in Supabase
  try {
    ...
    if (cached && cached.lat && cached.long) {
      return { lat: Number(cached.lat), long: Number(cached.long) };
    }
  } catch (err) {
